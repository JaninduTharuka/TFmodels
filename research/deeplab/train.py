...
# Disable TF2 behaviors and make tf reference point to compat.v1 for minimal changes.
_tf_orig.compat.v1.disable_v2_behavior()
# Rebind tf to compat.v1 so existing TF1.x calls work with minimal change.
tf = _tf_orig.compat.v1

# Try to import contrib modules safely; if unavailable, provide fallbacks.
try:
...
try:
    from tensorflow.contrib import tfprof as contrib_tfprof  # type: ignore
except Exception:
    contrib_tfprof = None

# slim: prefer standalone tf_slim, otherwise try contrib.slim
try:
...
# Provide a dummy profile context if tfprof isn't available so code path still runs.
...
ProfileContext = contrib_tfprof.ProfileContext if contrib_tfprof is not None else _dummy_profile_context

# Flags: use compat.v1 app.flags
flags = tf.app.flags
FLAGS = flags.FLAGS

# --- (Keep all flags from original file) ---
...
flags.DEFINE_integer('hard_example_mining_step', 0, 'The training step in which exact hard example mining kicks off. Note we gradually reduce the mining percent to the specified top_k_percent_pixels.')
